---
title: "genome_resequencing_notebook_hvol"
...
and it may overrite your files if you do. 
```{r}
'''
data = read_csv("Paper/multi_h_volcanii_compare_table_gdtools_paper_final.csv") %>%
  mutate(mut_id = row_number()) %>%
  mutate_at(vars(contains("hv_")), as.numeric) %>%
  rowwise() %>%
  mutate(strains_present_breseq = sum(c_across(starts_with("hv_")), na.rm=TRUE)) %>%
  ungroup() %>%
  select(-starts_with("hv_"))

write_csv(data, "Paper/h_volcanii_mutations.csv")
rm(data)

data = read_csv("Paper/multi_h_volcanii_compare_table_gdtools_paper_final.csv") %>%
  select(starts_with("hv")) 
strains_data = read_xlsx("Paper/supplemntary_table_strains-AKS_final.xlsx") %>%
  filter(Species == "Haloferax volcanii") #Because both species are present in this file
...
write_csv(data, "Paper/h_volcanii_mutations_raw_observations.csv")
'''
...
data = read_csv("Paper/h_volcanii_mutations_final.csv") %>%
  select(-c(type, seq_id, position, mutation, annotation, gene, old_gene_name, description,strains_present_breseq,strains_present_manual_annotation,Expected_KO,mut_type,is_mobile_element, Prevalence))

data_long = pivot_longer(data, cols=-mut_id, names_to="strain", values_to = "presence")


all_mutations = read_csv("Paper/h_volcanii_mutations_final.csv") %>%
...
#Create a heatmap of data_heatmap
  ...
  non_italics_list = c("H1209_RP", "DS2_AB","H26_AKS","H26_JMF","HVO_1559","HVO_2423","HVO_2510","HVO_2734","HVO_2795","HVO_2886") #Things not identified by their knockout
  colnames = as.expression(lapply(colnames(data_heatmap_restored),
   function(x) 
   if (!x %in% non_italics_list) bquote(paste("\U0394",italic( .(x)), sep=""))
   else if (x == "DS2_AB") x
   else paste("\U0394", x))) #Make the column names italics, except for things in non_italics_list
  
  colC = left_join(tibble(Common_Name = colnames(data_heatmap_restored)), select(strains_data, Common_Name, Lab_Strain)) %>%
    left_join(tibble(Lab_Strain=c("Schmid","Bisson", "Daniels", "Marchfelder", "Maupin-Furlow", "Peck", "Schönheit"), Color = palette.colors(7))) 
  ...
  gplots::heatmap.2(as.matrix(data_heatmap_restored), trace="none", key=FALSE, Rowv = rowDend, Colv = colDend, sepcolor="black",
   col = colorRampPalette(c("#BABABA","#D53E4F","#3288BD"))(3),cexCol=1, margins=c(10,10),key.ylab=NA, labRow = FALSE, labCol = colnames,
   ColSideColors=colC$Color, RowSideColors = rowC$Color, xlab="Strain",ylab="Mutation")
...
```{r}
annotations = read_csv("h_vol_KO_annotations.csv") %>%  #Renaming annotations to common names
  mutate(chr = ifelse(chr == "NC_013967", "Chromosome", chr)) %>%
  mutate(chr = ifelse(chr == "NC_013968", "pHV1", chr)) %>%
  mutate(chr = ifelse(chr == "NC_013965", "pHV2", chr)) %>%
  mutate(chr = ifelse(chr == "NC_013964", "pHV3", chr))%>%
  mutate(chr = ifelse(chr == "NC_013966", "pHV4", chr))

muts = rename(all_mutations, chr = "seq_id") %>%  #Cleaning annotations in muts file to be consistent with annotations file
  mutate(chr = ifelse(chr == "NC_013967", "Chromosome", chr)) %>%
  mutate(chr = ifelse(chr == "NC_013968", "pHV1", chr)) %>%
  mutate(chr = ifelse(chr == "NC_013965", "pHV2", chr)) %>%
  mutate(chr = ifelse(chr == "NC_013964", "pHV3", chr))%>%
...
muts_cod_global = filter(muts, mut_type == "coding", Prevalence=="Global") %>% 
  #mutate(position_y = runif(nrow(.), .1, .9))
  mutate(position_y = c(.01, .25, .25, .5, .75, .99, .75, .5))  #Manually setting y positions for better visualization

muts_syn_ura_minus = filter(muts, mut_type == "synonymous", Prevalence=="Parental") %>%
  #mutate(position_y = runif(nrow(.), .1,.9))
  mutate(position_y = c(.275, 0, .15, .32, .444, .763))
...
muts_cod_ura_minus = filter(muts, mut_type == "coding", Prevalence=="Parental") %>%
  #mutate(position_y = runif(nrow(.), .1,.9))
  mutate(position_y =c(.730, .453, .49, .66, .83, .99, .661, .866, .666))

muts_cod_KO = filter(muts, mut_type=="coding", Prevalence=="Knockout") %>%
  #mutate(position_y = runif(nrow(.), .1,.9))
  mutate(position_y = c(0.559, 0.718, .01, 0.2, 0.4, 0.6, 0.8, 0.206, 0.99, 0.331,
   0.559, 0.354, 0.713, 0.668, 0.437, 0.890, 0.95, 0.670, 0.258, 0.2, 0.837))
muts_syn_KO = filter(muts, mut_type=="synonymous", Prevalence=="Knockout") %>%
  #mutate(position_y = runif(nrow(.), .1,.9))
  mutate(position_y = c(.733, .546, .448, .747, .188, .724, .308))
muts_non_KO = filter(muts, mut_type=="noncoding", Prevalence=="Knockout") %>%  
  #mutate(position_y = runif(nrow(.), .1,.9))
  mutate(position_y = c(.55, .902, .331, .259, .85, .690, .222, .281, .882, .183,
   .164, .483, .01, .33, .66, .99, .858, .775, .426, .263,
   .821, .571, .879))
...
genome_coords = matrix(c(rep(0,4), c(2847757,85092,437906,635786)),ncol=2) #Setting up the genome coordinates

col_coding = "#E7298A"
col_noncoding = "#A6761D"
col_synonymous = "#666666"

lgd_points_col = Legend(at = c("mobileome", "non-mobileome", "coding", "noncoding", "synonymous"), #Building the legend
  type="points", pch=c(15,16,16,16,16), title="Points", legend_gp = gpar(col=c("black","black",col_coding,col_noncoding,col_synonymous)))
...
circos.initialize(factors=c("Chromosome","pHV1","pHV3","pHV4"),xlim=genome_coords) #Initializes the plot with the 4 chromosomes
...
#Old colors
#col_coding = "#1B9E77"
#col_noncoding = "darkorchid4"
#col_synonymous = "#D95F02"


#Plotting class 1 mutations
circos.trackPoints(muts_cod_global_TE$chr, x=muts_cod_global_TE$position,y=muts_cod_global_TE$position_y, col=col_coding, pch=15)
circos.trackPoints(muts_cod_global_non_TE$chr, x=muts_cod_global_non_TE$position,y=muts_cod_global_non_TE$position_y, col=col_coding, pch=16)
circos.trackPoints(muts_syn_global$chr, x=muts_syn_global$position,y=muts_syn_global$position_y, col=col_synonymous, pch=16)
circos.trackPoints(muts_non_global$chr, x=muts_non_global$position,y=muts_non_global$position_y, col=col_noncoding, pch=16)
circos.trackPoints(muts_non_global_TE$chr, x=muts_non_global_TE$position,y=muts_non_global_TE$position_y, col=col_noncoding, pch=15)

circos.track(ylim=c(0,1), panel.fun=function(x,y){ #Setting up the class 2 mutation track
}, track.height=.1)
#Plotting class 2 mutations
circos.trackPoints(muts_cod_ura_minus_TE$chr, x=muts_cod_ura_minus_TE$position,y=muts_cod_ura_minus_TE$position_y, col=col_coding, pch=15)
circos.trackPoints(muts_cod_ura_minus_non_TE$chr, x=muts_cod_ura_minus_non_TE$position,y=muts_cod_ura_minus_non_TE$position_y, col=col_coding, pch=16)
circos.trackPoints(muts_syn_ura_minus$chr, x=muts_syn_ura_minus$position,y=muts_syn_ura_minus$position_y, col=col_synonymous, pch=16)
circos.trackPoints(muts_non_ura_minus_TE$chr, x=muts_non_ura_minus_TE$position,y=muts_non_ura_minus_TE$position_y, col=col_noncoding, pch=15)
circos.trackPoints(muts_non_ura_minus_non_TE$chr, x=muts_non_ura_minus_non_TE$position,y=muts_non_ura_minus_non_TE$position_y, col=col_noncoding, pch=16)

circos.track(ylim=c(0,1), panel.fun=function(x,y){ #Setting up the class 3 mutation track
}, track.height=.1)
#Plotting class 3 mutations
circos.trackPoints(muts_cod_KO$chr, x=muts_cod_KO$position,y=muts_cod_KO$position_y, col=col_coding, pch=16)
circos.trackPoints(muts_syn_KO$chr, x=muts_syn_KO$position,y=muts_syn_KO$position_y, col=col_synonymous, pch=16)
circos.trackPoints(muts_non_KO$chr, x=muts_non_KO$position,y=muts_non_KO$position_y, col=col_noncoding, pch=16)

circos.genomicLabels(annotations, labels.column=5, side="inside", cex=.75) #Adding the gene annotations
