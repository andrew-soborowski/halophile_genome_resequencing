...
```{r}
''' # nolint
data = read_csv("Paper/multi_h_salinarum_compare_table_gdtools_paper_final.csv") %>%
...
write_csv(data, "Paper/h_salinarum_mutations.csv")
rm(data)
...
write_csv(data, "Paper/h_salinarum_mutations_raw_observations.csv")
'''
...
data = read_csv("Paper/h_salinarum_mutations_final.csv") %>%
...
all_mutations = read_csv("Paper/h_salinarum_mutations_final.csv") %>%
...
muts_cod_global = filter(muts, mut_type == "coding", Prevalence=="Global") %>% 
  #mutate(position_y = runif(nrow(.), .1, .9))
  mutate(position_y = c(.01, .33, .66, .99, .5, .5, .5, .5))  #Manually doing these cause they are too close together to trust RNG
